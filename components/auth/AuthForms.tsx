...
function traducirError(code: string): string {
  const errores: Record<string, string> = {
    ...
    "auth/cancelled-popup-request": "Ya hay otra ventana abierta.",
...
export default function AuthForms() {
  ...
  const handleGoogleSignIn = async () => {
    ...
    try {
      ...
      // Guardar info básica del usuario si es nuevo
      ...
      if (!userSnap.exists()) {
        ...
        await setDoc(userRef, {
          ...
          username: suggestedUsername || "usuario",
      ...
      const mensaje = errorCode ? traducirError(errorCode) : error?.message || "No se pudo iniciar sesión.";
      setMensaje(mensaje);
  ...
  return (
    <div className="space-y-4 rounded-2xl border border-gray-200 bg-white p-4 shadow-sm">
      ...
      <button
        ...
        className="flex w-full items-center justify-center gap-3 rounded-xl border-2 border-gray-200 bg-white px-4 py-3 font-semibold transition hover:bg-gray-50 disabled:opacity-60"
        style={{ color: "#111827" }}
      >
        {oauthLoading ? (
      ...
      <div className="relative">
        ...
        <div className="relative flex justify-center text-sm">
          <span className="bg-white px-4 text-gray-500">o con correo</span>
        </div>
      </div>
